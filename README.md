...
It also **checks** the delivered amount rather than only recording it, and emits a
`DoseWarning` when a draw lands outside the band `dose_tolerance` allows for that
condition. It warns and carries on; it does not stop. The band is three standard
errors, so a working injector trips it by chance — on a few thousand records with a
heavy tail, roughly 1% of draws — and stopping there would discard a sound run over
an unlucky draw. Nothing is lost by continuing, because `realised_dose_label_units`
is on every row beside the amount requested, so a run can be filtered afterwards.

One warning is expected. **The same offset on every draw is not**, and that is what
the check exists to catch: the defect it was written for was a fixed scale error of
√2 present on every single draw.

`DoseError` is defined and exported for a caller that wants a missed draw to be
